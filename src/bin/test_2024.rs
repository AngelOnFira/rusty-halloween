...
fn main() {
    ...
    let song: Value =
        serde_json::from_str(&song_instructions).expect("Failed to parse song instructions");
    ...
    for (timestamp, frame) in song.as_object().unwrap() {
        ...
        for (device, _) in frame.as_object().unwrap() {
            if !hardware.as_object().unwrap().contains_key(device) {
                println!(
                    "Warning: Device '{}' in song instructions not found in hardware config",
                    device
                );
            }
        }
    ...
    if let Some(first_frame) = show.frames.first() {
        ...
        //     println!(
        //         "  {}: Channel {} - {:?}",
        //         dmx_state.device_name, dmx_state.channel_id, dmx_state.values
        //     );
        // }
    }
    ...
    if let Some(last_frame) = show.frames.last() {
        println!(
            ...
            last_frame.timestamp as f64 / 1000.0
        );
    }
}
